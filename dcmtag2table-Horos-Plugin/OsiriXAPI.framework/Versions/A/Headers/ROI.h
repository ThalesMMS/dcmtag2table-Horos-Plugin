...
@interface ROI : NSObject <NSCoding, NSCopying>
{
    ...
    BOOL            mouseOverROI, roiToolActive;
	int				pointUnderMouse;
    ...
    BOOL notifyOnRelease, postNotifications;
    
    int thickSlabCount, thickSlabMode;
}
...
@property int textureDownRightCornerX,textureDownRightCornerY, textureUpLeftCornerX, textureUpLeftCornerY;
...
@property BOOL isAliased, displayCMOrPixels, roiToolActive, mouseOverROI, dontDisplayInKeyImagesWindow, fill, is3DMeasure, textNameOnly;
@property int thickSlabCount, thickSlabMode, pointUnderMouse, middleIndex;
...
@property BOOL dontSavePreferredWLWW, postNotifications, notifyOnRelease;
...
+ (unsigned char*) brushCursorForSize: (int) size;
+ (unsigned char*) dilateAndFrameWithMargin: (int) margin texture:(unsigned char*) textureBuffer width: (int) width height:(int) height;
- (void) lock;
- (void) unlock;
- (void) newUID;
- (void) setDefaultName:(NSString*) n;
...
- (void) setObservers;
- (void) setPoints: (NSArray <MyPoint*> *) pts;
...
- (void) rotateTexture90:(int) rotationConstant center: (NSPoint) pt;
...
+ (void) deleteROIs: (NSArray*) array postNotification: (BOOL) postNotification evenIfLocked: (BOOL) evenIfLocked;
...
- (long) clickInROI:(NSPoint) pt offsetx:(float) offsetx offsety:(float) offsety scale:(float) scale testDrawRect:(BOOL) testDrawRect radius: (float) NEIGHBORHOODRADIUS includeHidden:(BOOL) includeHidden;
...
- (unsigned long) textureBufferSize;
...
- (NSPoint) lowerRightCoordinate;
- (NSPoint) upperLeftCoordinate;
