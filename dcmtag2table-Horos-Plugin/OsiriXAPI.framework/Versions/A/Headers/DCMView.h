...
#define INVALID_FLOAT 1234567890
...
@class ScreenCapture;
@class ROINamePopOver;

/** \brief Image/Frame View for ViewerController */

@interface DCMView: NSOpenGLView <NSPasteboardItemDataProvider, NSDraggingSource>
{
    ...
    float           thumbnailCrossIndex0[ 2][ 3], thumbnailCrossIndex1[ 2][ 3];
	...
	GLString		*showDescriptionInLargeText, *flippedNotice, *ROIsHiddenNotice, *priorNotice, *todayNotice, *latestNotice;
...
//	LoupeController *loupeController;
    ...
    NSTimeInterval lastBecomeMainWindowTime, lastCineSyncTimeInterval;
...
#define PBO_COUNT 2
    ...
    double resliceViewOrientation[ 9];
    ...
    BOOL persitentLensMode, cursorDisplayedWithName;
    ...
    BOOL wasScaledToFit, clickedInSameROI;
    
    ToolMode toolBeforeScreenCapture;
    NSEventType previousClickCountEventType;
}

@property BOOL flippedRendered, displayResliceView, mouseDownInOpenGLSlider;
...
@property(retain) ROI *curROI, *mouseDownROI;
...
@property BOOL drawing, newTextROICreated;
@property (readonly) BOOL volumicSeries;
@property (nonatomic) NSTimeInterval timeIntervalForDrag;
@property (readonly) NSTimeInterval lastCineSyncTimeInterval;
@property(readonly) BOOL isKeyView, mouseDragging;
@property annotationsLevel annotationType;
@property(readonly) int volumicData;
@property short thickSlabStacks, thickSlabMode;
@property(retain) NSString *thumbnailCrossObject0, *thumbnailCrossObject1;
@property(retain) ScreenCapture *screenCapture;
@property(retain) ROINamePopOver *roiNamePopOver;
@property(retain) NSTimer *rightClickTimer;
...
+ (NSPoint) projectPoint: (NSPoint) p ontoLine:(NSPoint) a :(NSPoint) b;
...
- (short)syncro;
- (void)setSyncro:(short) s;
- (void) annotationsSwitch: (id) sender;
...
- (NSRect) ConvertRectFromGL2NSView:(NSRect) a;
- (NSRect) smartCrop;
...
- (void) setIndexWithReset:(short) index sizeToFit:(BOOL) sizeToFit;
...
- (BOOL) isScaledFit;
- (BOOL) haveAllPixSameOrientationTo: (NSArray*) a;
...
- (void) createAliasesOnAdjacentSlicesForROI: (ROI*) roi;
...
- (int) displayedImageIndexOfROI:(ROI*) c;
- (int) imageIndex: (int) i;
- (int) middleIndexOfROIName: (NSString*) n;
- (int) middleIndexOfROIName: (NSString*) n arrayOfROIsNames: (NSArray*) names;
- (NSArray*) arrayOfROIsNames;
- (void) roiDeleteAll;
...
- (void) setMouseThumbnailCrossReferences:(NSDictionary*) p index: (int) i;

- (IBAction) screenCaptureIcon:(NSToolbarItem *) tbi;
- (void) screenCaptureEnded: (ScreenCapture*) capture;
