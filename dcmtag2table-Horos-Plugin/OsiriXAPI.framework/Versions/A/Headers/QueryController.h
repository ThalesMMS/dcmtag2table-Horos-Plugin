...
@interface QueryController : NSWindowController <NSWindowDelegate, NSOutlineViewDelegate, NSTableViewDelegate, NSMenuDelegate>
{
    ...
    IBOutlet    NSSegmentedControl          *segmentedActions;
    IBOutlet    DNDArrayController          *sourcesArrayController;
...
@property BOOL autoQuery, DatabaseIsEdited, sourceNodeCMove;
...
+ (NSArray*) queryStudiesForPatient:(DicomStudy*) study usePatientID:(BOOL) usePatientID usePatientName:(BOOL) usePatientName usePatientBirthDate: (BOOL) usePatientBirthDate servers: (NSArray*) serversList showErrors: (BOOL) showErrors error:(NSError**) error;
...
+ (NSUInteger) indexOfCorrespondingServer: (NSDictionary*) savedServer inServers : (NSArray*) servers;
...
- (NSArray*) queryAccessionNumber:(NSString*) ID;
- (NSArray*) queryStudyInstanceUID:(NSString*) ID;
...
- (void) performRetrieve:(NSDictionary*) array;
