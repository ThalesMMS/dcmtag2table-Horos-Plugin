...
extern "C"
{
    ...
    extern NSString* getSerialAddress(void);
...
+ (NSString*) getRK;
+ (void) restartOsiriX;
+ (void) restartOsiriXWithTerminate: (BOOL) terminate;
...
- (int) currentColumnForViewer: (ViewerController*) v;
...
- (void) computeViewersRowColumn;
- (void) rows: (int*) rows columns: (int*) columns forScreen: (NSScreen*) screen;
...
+ (void) refreshOAuthForServer:(NSDictionary*) server;
